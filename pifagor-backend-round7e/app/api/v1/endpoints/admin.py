...
async def list_receipts(db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(EmailReceipt)
        .options(
            joinedload(EmailReceipt.child).joinedload(ChildProfile.user),
            joinedload(EmailReceipt.splits).joinedload(EmailReceiptSplit.child).joinedload(ChildProfile.user),
        )
...
async def assign_receipt_children(
    ...
    split_children = [
        {"child_id": s.child_id, "name": f"{s.child.user.last_name} {s.child.user.first_name}".strip(), "amount": s.amount}
        for s in r.splits if s.child and s.child.user
    ]
...
async def finance_report(
    ...
    # Доли из чеков, разделённых между несколькими учениками (например,
    # брат и сестра, оплаченные одним платежом) — тот же фильтр по дате
    # для перенесённых учеников применяется и здесь.
    split_filters = []
    if week_start and week_end:
        split_filters.extend([
            cast(EmailReceipt.payment_date, Date) >= week_start,
            cast(EmailReceipt.payment_date, Date) <= week_end,
        ])
    splits_res = await db.execute(
        select(EmailReceiptSplit.child_id, func.sum(EmailReceiptSplit.amount).label("total"))
        .join(EmailReceipt, EmailReceiptSplit.receipt_id == EmailReceipt.id)
        .join(ChildProfile, EmailReceiptSplit.child_id == ChildProfile.id)
        .join(User, ChildProfile.user_id == User.id)
        .where(
            *split_filters,
            or_(
        ...
        .group_by(EmailReceiptSplit.child_id)
    )
    for row in splits_res:
        amounts_by_child[row.child_id] = (amounts_by_child.get(row.child_id, 0) or 0) + (row.total or 0)
