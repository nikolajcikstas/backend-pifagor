...
# Формат хэша bcrypt самоописывающийся, поэтому свежий CryptContext здесь
# спокойно проверяет пароли, захэшированные где угодно ещё в системе
# (например, при регистрации через app.core.security.get_password_hash).
...
async def forgot_password(
    ...
    """Публичный эндпоинт: восстановление пароля по номеру телефона +
    email, без ввода старого пароля. Если оба значения совпадают с
    учётной записью — пароль меняется сразу."""
    if len(body.new_password) < 6:
        raise HTTPException(status_code=400, detail="Пароль минимум 6 символов")
