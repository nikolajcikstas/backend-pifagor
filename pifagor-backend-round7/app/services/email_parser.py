...
async def _apply_payer_match(db: AsyncSession, receipt) -> bool:
    ...
    быть id). Если плательщик привязан к нескольким ученикам — делит сумму
    поровну между ними в email_receipt_splits, а receipt.child_id остаётся
    пустым (сумма распределена, не привязана к одному). Возвращает True,
    если что-то поменялось."""
    from app.models.models import EmailReceiptSplit

    child_ids = await _resolve_payer_children(receipt.payer_name, db)

    existing_splits_res = await db.execute(
        select(EmailReceiptSplit).where(EmailReceiptSplit.receipt_id == receipt.id)
    )
    existing_splits = existing_splits_res.scalars().all()
    existing_split_child_ids = sorted(s.child_id for s in existing_splits)

    new_child_id = child_ids[0] if len(child_ids) == 1 else None
    new_split_child_ids = sorted(child_ids) if len(child_ids) > 1 else []

    changed = (
        receipt.child_id != new_child_id
        or existing_split_child_ids != new_split_child_ids
    )
    if not changed:
        return False
    ...
    if len(child_ids) > 1:
        share = round(receipt.amount / len(child_ids), 2)
        for cid in child_ids:
            db.add(EmailReceiptSplit(receipt_id=receipt.id, child_id=cid, amount=share))
