...
async def list_published_tutors_compat(
    db: AsyncSession = Depends(get_db),
    _: User = Depends(require_admin),
):
    # Список нужен только админу (выпадающий список репетиторов при создании
    # занятия). Раньше был открыт всем без авторизации и отдавал email и
    # ставки репетиторов любому человеку из интернета.
...
async def list_students_for_tutor_compat(
    db: AsyncSession = Depends(get_db),
    _: User = Depends(require_admin),
):
    # Раньше был открыт без авторизации — любой мог получить имена и email
    # всех учеников. Теперь только для админа.
...
@router.get("/tutors/public", response_model=list[dict])
async def list_published_tutors(
    db: AsyncSession = Depends(get_db),
    _: User = Depends(require_admin),
):
