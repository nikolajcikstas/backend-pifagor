...
from app.services.email_parser import _find_child_by_payer_name, _normalize_name
from app.services.tutor_earnings import compute_tutor_earnings, get_current_and_pending_rate
...
async def list_receipts(db: AsyncSession = Depends(get_db)):
    ...
    # Плательщики, привязанные сразу к нескольким ученикам (семьи) —
    # чтобы показать их имена вместо «не сопоставлен».
    links_res = await db.execute(
        select(PayerChildLink).options(joinedload(PayerChildLink.child).joinedload(ChildProfile.user))
    )
    group_names: dict[str, list] = {}
    group_counts: dict[str, int] = {}
    for link in links_res.scalars().unique().all():
        group_counts[link.payer_name_normalized] = group_counts.get(link.payer_name_normalized, 0) + 1
        if link.child and link.child.user:
            u = link.child.user
            group_names.setdefault(link.payer_name_normalized, []).append(
                {"child_id": link.child_id, "name": f"{u.last_name} {u.first_name}".strip(), "amount": None}
            )
    families = {key for key, count in group_counts.items() if count > 1}
    ...
    for r in receipts:
        ...
        split_children = group_names.get(norm, []) if norm in families else []
        ...
        output.append(EmailReceiptOut(
            ...
            is_unmatched=r.child_id is None and not split_children,
...
class ReceiptAssignChildren(BaseModel):
    child_ids: List[int]


@router.post("/receipts/{receipt_id}/assign-children", dependencies=[Depends(require_admin)])
async def assign_receipt_children(
    receipt_id: int, payload: ReceiptAssignChildren, db: AsyncSession = Depends(get_db)
):
    """Вручную привязать чек к одному или нескольким ученикам (например,
    брат и сестра, оплаченные одним платежом — сумма поделится поровну).
    Заодно запоминает эту привязку для плательщика — чтобы в будущем чеки
    от него сопоставлялись автоматически, без повторной ручной работы."""
    from app.services.email_parser import _apply_payer_match, _normalize_name
    ...
    result = await db.execute(select(EmailReceipt).where(EmailReceipt.id == receipt_id))
    ...
    normalized = _normalize_name(receipt.payer_name)
    existing_links_res = await db.execute(
        select(PayerChildLink.child_id).where(PayerChildLink.payer_name_normalized == normalized)
    )
    existing_link_ids = {row[0] for row in existing_links_res.all()}
    for cid in payload.child_ids:
        if cid not in existing_link_ids:
            db.add(PayerChildLink(payer_name_normalized=normalized, child_id=cid))
    await db.flush()

    # Применяем эту привязку не только к текущему чеку, но и ко всем другим
    # чекам от того же плательщика (включая более старые, уже лежавшие
    # непривязанными) — чтобы одна ручная привязка закрывала всю историю.
    all_receipts_res = await db.execute(select(EmailReceipt))
    for other_receipt in all_receipts_res.scalars().all():
        if _normalize_name(other_receipt.payer_name) == normalized:
            await _apply_payer_match(db, other_receipt)
    await db.commit()

    result = await db.execute(
        select(EmailReceipt)
        .options(joinedload(EmailReceipt.child).joinedload(ChildProfile.user))
        .where(EmailReceipt.id == receipt_id)
    )
    r = result.unique().scalar_one()
    student_name = f"{r.child.user.last_name} {r.child.user.first_name}".strip() if r.child and r.child.user else None

    group_names_res = await db.execute(
        select(PayerChildLink).options(joinedload(PayerChildLink.child).joinedload(ChildProfile.user))
        .where(PayerChildLink.payer_name_normalized == normalized)
    )
    group_links = group_names_res.unique().scalars().all()
    split_children = [
        {"child_id": l.child_id, "name": f"{l.child.user.last_name} {l.child.user.first_name}".strip(), "amount": None}
        for l in group_links if l.child and l.child.user
    ] if len(group_links) > 1 else []

    return EmailReceiptOut(
        id=r.id, receipt_number=r.receipt_number, payer_name=r.payer_name, amount=r.amount,
        payment_date=r.payment_date, child_id=r.child_id, student_name=student_name,
        is_unmatched=r.child_id is None and not split_children,
        split_children=split_children, created_at=r.created_at,
    )
...
async def update_admin_student(
    ...
    return {
        ...
        "accounting_start_date": (
            student.child_profile.accounting_start_date.isoformat()
            if student.child_profile.accounting_start_date else None
        ),
...
async def _mark_child_as_active_client(db: AsyncSession, child_id: int) -> None:
    """При успешной привязке договора переводим ученика в CRM-статус «Клиент»
    (это и есть статус подтверждённого активного ученика — отдельного
    статуса «Действующий» в CRM не предусмотрено)."""
    child_res = await db.execute(select(ChildProfile).where(ChildProfile.id == child_id))
    child = child_res.scalar_one_or_none()
    if child and child.crm_status != "Клиент":
        child.crm_status = "Клиент"
...
async def _process_one_contract_file(
    filename: str, contents: bytes, db: AsyncSession
) -> dict:
    """Общая логика обработки одного файла договора — используется и при
    одиночной, и при массовой загрузке. Бросает ValueError с понятным
    сообщением при любой проблеме, чтобы массовая загрузка могла просто
    пропустить этот файл и продолжить со следующим."""
    ...
    # Файл договора больше нигде не сохраняется (ни на диске, ни в БД) —
    # только данные, распознанные из его текста. Разбираем прямо из памяти,
    # без записи на диск.
    try:
        parsed = parse_contract_docx(io.BytesIO(contents))
    except Exception as e:
        raise ValueError(f"не удалось разобрать договор: {e}")
    ...
    if match_name:
        ...
        if child_id:
            ...
            await _mark_child_as_active_client(db, child_id)
        else:
    ...
    contract = ParentContract(
        parent_id=parent_id,
        child_id=child_id,
        # Сам файл больше не хранится — сохраняем только его исходное имя
        # (для истории/логов), колонка file_url в схеме обязательная.
        file_url=filename or "contract.docx",
        is_signed=True,
...
async def upload_contract(file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
    ...
    try:
        row = await _process_one_contract_file(file.filename or "", contents, db)
    except Exception as e:
        await db.rollback()
...
@router.post("/contracts/upload-bulk", dependencies=[Depends(require_admin)])
async def upload_contracts_bulk(
    files: List[UploadFile] = File(...), db: AsyncSession = Depends(get_db)
):
    """Загрузить несколько договоров разом. Файлы, которые не получилось
    разобрать, пропускаются — остальные загружаются как обычно."""
    created = []
    failed = []
    for f in files:
        contents = await f.read()
        try:
            row = await _process_one_contract_file(f.filename or "", contents, db)
            created.append(row)
        except Exception as e:
            await db.rollback()
...
@router.post("/contracts/manual", dependencies=[Depends(require_admin)])
async def create_manual_contract(db: AsyncSession = Depends(get_db)):
    """Создать пустой договор для ручного заполнения — для случаев, когда
    файл .docx не удаётся разобрать автоматически. Дальше все поля
    заполняются прямо в таблице, через тот же 'Сохранить правки', что и
    у обычных распознанных договоров."""
    contract = ParentContract(
        file_url="",
        is_signed=False,
        match_status="unmatched",
...
async def update_contract_fields(
    ...
    """Ручная правка полей договора — как для полей, которые распознал
    парсер и ошибся, так и для договора, заполненного полностью вручную
    (когда файл вообще не удалось разобрать)."""
    result = await db.execute(select(ParentContract).where(ParentContract.id == contract_id))
    contract = result.scalar_one_or_none()
    if not contract:
        raise HTTPException(status_code=404, detail="Договор не найден")
    ...
    # Если указали ФИО родителя, а договор ещё ни к кому не привязан —
    # пробуем сопоставить с учеником автоматически (та же логика, что и
    # для чеков об оплате). Не получится — можно привязать вручную, как обычно.
    if "parent_full_name" in updates and updates["parent_full_name"] and contract.child_id is None:
        child_id = await _find_child_by_payer_name(updates["parent_full_name"], db)
        if child_id:
            ...
            parent_link = parent_link_res.scalars().first()
            if parent_link:
                contract.parent_id = parent_link.parent_id
            contract.match_status = "matched"
            await _mark_child_as_active_client(db, child_id)

    # Как только заполнены все основные поля — договор считается
    # оформленным, статус переключается на «Действующий».
    core_fields = [
        contract.start_date, contract.end_date, contract.parent_full_name,
        contract.parent_phone, contract.city, contract.street, contract.house,
    ]
    if all(bool(v) for v in core_fields):
...
def _serialize_tutor(
    tutor: TutorProfile,
    earnings: Optional[float] = None,
    pending_rate: Optional[float] = None,
    pending_rate_from: Optional[date] = None,
) -> TutorProfileOut:
    ...
    return TutorProfileOut(
        ...
        pending_rate_from=pending_rate_from,
    )
...
async def _tutor_earnings_map(
    ...
    """Сколько должны каждому репетитору: (проведённые занятия * ставка,
    действовавшая на дату каждого занятия) - уже выплачено.
    ...
    earned_by_tutor: dict[int, float] = {}
    for tutor_id in tutor_ids:
        total_earned, _count = await compute_tutor_earnings(db, tutor_id, as_of_date=as_of)
        earned_by_tutor[tutor_id] = total_earned
    ...
    result = {}
    for tutor_id in tutor_ids:
        total_earned = earned_by_tutor.get(tutor_id, 0)
...
class TutorRateChange(BaseModel):
    rate_per_hour: float
    effective_from: date


@router.post(
    "/tutors/{tutor_id}/rate-change",
    response_model=TutorProfileOut,
    dependencies=[Depends(require_admin)],
)
async def change_tutor_rate(
    tutor_id: int,
    payload: TutorRateChange,
    db: AsyncSession = Depends(get_db),
):
    """Изменить ставку репетитора с конкретной даты — занятия до этой даты
    по-прежнему считаются по старой ставке, начиная с неё — по новой.
    Не пересчитывает задним числом уже проведённые занятия."""
    ...
    if payload.rate_per_hour <= 0:
        raise HTTPException(status_code=400, detail="Ставка должна быть больше нуля")

    # Если у репетитора ещё нет истории ставок — сначала фиксируем его
    # ТЕКУЩУЮ ставку как действовавшую с самого раннего занятия (или с
    # сегодня, если занятий ещё не было), чтобы прошлые занятия не съехали
    # на новую ставку задним числом.
    existing_res = await db.execute(
        select(TutorRateHistory.id).where(TutorRateHistory.tutor_id == tutor_id)
    )
    has_history = existing_res.first() is not None
    if not has_history and tutor.rate_per_hour:
        earliest_lesson_res = await db.execute(
            select(func.min(Lesson.date)).where(Lesson.tutor_id == tutor_id)
        )
        earliest_date = earliest_lesson_res.scalar() or date.today()
        db.add(TutorRateHistory(
            tutor_id=tutor_id,
            rate_per_hour=tutor.rate_per_hour,
    ...
    # tutor.rate_per_hour отражает ставку, действующую СЕГОДНЯ — если новая
    # ставка ещё не наступила (дата в будущем), поле не меняется, а новая
    # ставка видна отдельно как "запланированная" (pending_rate).
    current_rate, pending_rate, pending_rate_from = await get_current_and_pending_rate(
        db, tutor_id, tutor.rate_per_hour
    )
    tutor.rate_per_hour = current_rate
    await db.commit()

    result = await db.execute(
        select(TutorProfile).options(
            joinedload(TutorProfile.user),
            joinedload(TutorProfile.subjects).joinedload(TutorSubject.subject),
    ...
    return _serialize_tutor(
        tutor, earnings=earnings_map.get(tutor.id, 0.0),
        pending_rate=pending_rate, pending_rate_from=pending_rate_from,
    )
...
async def update_admin_tutor(
    ...
    tutor = result.unique().scalar_one()
    earnings_map = await _tutor_earnings_map(db, [tutor.id])
    _cur, pending_rate, pending_rate_from = await get_current_and_pending_rate(db, tutor.id, tutor.rate_per_hour)
    return _serialize_tutor(
        tutor, earnings=earnings_map.get(tutor.id, 0.0),
        pending_rate=pending_rate, pending_rate_from=pending_rate_from,
    )
...
# ─── Отчёт: ученики без занятий N дней (выгрузка в Excel) ──────────────────────

@router.get("/reports/inactive-students.xlsx", dependencies=[Depends(require_admin)])
async def inactive_students_report(
    days: int = Query(5, description="Порог в днях с последнего проведённого занятия"),
    db: AsyncSession = Depends(get_db),
):
    """Список учеников, у которых последнее занятие со статусом «проведено»
    было больше days дней назад (или таких занятий не было вообще) —
    чтобы понять, кто сейчас фактически не занимается. Отдаёт готовый
    .xlsx файл на скачивание."""
    ...
    rows = []
    for child, user in children:
        last_res = await db.execute(
            select(func.max(Lesson.date)).where(
        ...
        if last_date is None or last_date < cutoff:
            rows.append((
                f"{user.last_name} {user.first_name}".strip(),
                child.crm_status,
                last_date.isoformat() if last_date else "никогда",
                (date.today() - last_date).days if last_date else "",
            ))

    rows.sort(key=lambda r: (r[3] == "", -(r[3] if isinstance(r[3], int) else 0)))
...
async def email_diagnostics(_: User = Depends(require_admin)):
    """Проверка настройки почты для уведомлений о заявках с сайта — какие
    переменные окружения заполнены на сервере (сами значения, кроме адресов,
    не показываются) и пробная отправка письма. Помогает быстро понять,
    почему заявки не долетают на почту, не заглядывая в Render вручную."""
    import smtplib
    from app.services import email_notification as en

    env_status = {
        "EMAIL_USER": bool(en.EMAIL_USER),
        "EMAIL_USER_value": en.EMAIL_USER or None,
        "EMAIL_PASSWORD": bool(en.EMAIL_PASSWORD),
        "EMAIL_SMTP_HOST": en.SMTP_HOST,
        "EMAIL_SMTP_PORT": en.SMTP_PORT,
        "NOTIFICATION_EMAIL": bool(en.NOTIFICATION_EMAIL),
        "NOTIFICATION_EMAIL_value": en.NOTIFICATION_EMAIL or None,
    }

    if not en.EMAIL_USER or not en.EMAIL_PASSWORD or not en.NOTIFICATION_EMAIL:
        return {
            "configured": False,
            "env": env_status,
            "error": "Не заполнены переменные окружения EMAIL_USER / EMAIL_PASSWORD / NOTIFICATION_EMAIL на сервере (Render → pifagor-api → Environment).",
        }

    test_error = None
    try:
        with smtplib.SMTP(en.SMTP_HOST, en.SMTP_PORT, timeout=15) as server:
            server.starttls()
            server.login(en.EMAIL_USER, en.EMAIL_PASSWORD)
    except smtplib.SMTPAuthenticationError as e:
        test_error = (
            "Ошибка авторизации на почтовом сервере — неверный логин/пароль. "
            "Если это Gmail, EMAIL_PASSWORD должен быть паролем приложения "
            "(App Password, 16 символов), а не обычным паролем от аккаунта. "
            f"Подробности: {e}"
        )
    except Exception as e:
        test_error = f"Не удалось подключиться к почтовому серверу: {e}"

    return {
        "configured": test_error is None,
        "env": env_status,
        "error": test_error,
    }
